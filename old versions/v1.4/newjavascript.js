...
function lostLife(){
    if(leben != 1){
         ...
         shootPossible = false; //Schießen nicht möglich
         imgStick = document.getElementById("stick"); //Stick-Bild resetten
         resetPowerBall();
         if(level != "Käster"){
...
function choosePowerUp(){
     switch(state){
        ...
        case "powerBall":
            ...
            timeoutPowerBall = setTimeout(resetPowerBall,10000); //Reset nach 10 Sek.
            break;
        case "moreLifes":
        ...
        case "autopilot":
            ...
            timeoutAutopilot = setTimeout(function(){flip = false;clearInterval(intervalAutopilot);},10000);
    }
}
...
function newShoot(index,x,x2,y){  //KLASSEN DEKLARATION FÜR EINEN SCHUSS
    this.startShoot = function(){
        ...
        ctx.clearRect(x,y,2,10);
        ctx.clearRect(x2,y,2,10);
...
function collisionWithBlocks(index,xShoot1,xShoot2,yShoot){ //PRÜFT AUF KOLLISION MIT BLÖCKEN
    for(var j = 0; j < blocks.length;j++){
        if(yShoot <= blocks[j][1] + gridSizeBlocks){
            //var y = blocks[j][1] + gridSizeBlocks;
