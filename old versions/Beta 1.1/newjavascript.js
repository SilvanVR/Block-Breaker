...
function pause(){ //Cleared alle Intervalle
  ...
  for(var k = 0;k<powerUp.length;k++) clearInterval(powerUp[k]);
  for(var z = 0;z<intervalShoot.length;z++) clearInterval(intervalShoot[z]); 
...
function levelComplete(){
    if(blocks.length == 0){ //Wenn alle Blöcke zerstört wurden 
        ...
        if(difficulty == "Easy") startGame(32,10,20,10,385,20,leben,"Easy"),buttonStart();
        if(difficulty == "Normal") startGame(8,10,20,7,385,20,leben,"Normal"),buttonStart();
        if(difficulty == "Hard") startGame(2,2,10,3,393,20,leben,"Hard"),buttonStart();
        if(difficulty == "Insane") startGame(1,2,8,2,395,10,leben,"Insane"),buttonStart();
    }
}
function lostLife(){
    if(leben != 1){
         ...
         resetPowerUp("gun");
         clearCanvas2();
         if(level != "Käster") gridSizeBall = 3; //Speed resetten    
         if(level == 4) finalTimer = 0.1; sound.currentTime = 0; document.getElementById("score").innerHTML = 0;
...
function FinalTimer(){ //Timer, der immer größer werdend in der Mitte des Canvas gezeichnet wird
    ...
    if(t == 10) newTextPopUp("Highscore: 72.4s");
    if(t > 10){
        var x = canvas.width/2 - textPlace;    
        ctx.clearRect(x-10,0,650,300);
...
// ----------------------------- BALL -------------------------------------
function newBall(x,y,index){ //Objekt Definition eines neuen Balls
...
while(this.ball == DirectionBall && this.steigung == SteigungBall){ //Verhindert, dass neue Bälle niemals wie der Echte Ball fliegen
    ...
    this.startInterval = function(){
        this.interval = setInterval(function(){balls[index].move();},20);
    };
...
function choosePowerUp(st){
     switch(st){
        ...
        case "bomb":
            ...
            break;
        case "newBall":
            newTextPopUp("New Ball!");
            balls[indexBall] = new newBall(curPosBall['x'],curPosBall['y'],indexBall);
...
function shoot(){   //START FUNKTION UM EINEN SCHUSS ZU ERZEUGEN
    if(shootPossible){
        var shootObj = new newShoot(indexShoot,currentPositionStick['x'] + 0.5*stickLength,currentPositionStick['x'] + 9.5*stickLength,currentPositionStick['y'] - stickHeight);
  ...
  else 
  {
   ...
   switch(keyCode)
   {
        ...
        case 87: //W
            ...
            break;
        case 68: //D
            ctx.clearRect(blocks[0][0],blocks[0][1],2.3*gridSizeBlocks,1.3*gridSizeBlocks); //Lösche Block + Schatten
            blocks.splice(0,1);  
            levelComplete();
            break;
        default:
