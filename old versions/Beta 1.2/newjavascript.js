...
function lostLife(){
    if(leben != 1){
         ...
         if(level == 4){ finalTimer = 0.1; sound.currentTime = 0; document.getElementById("score").innerHTML = 0; intervalBall = [];}
...
function FinalTimer(){ //Timer, der immer größer werdend in der Mitte des Canvas gezeichnet wird
    ...
    if(t == 5) newTextPopUp("Reach 100s to defeat Käster!");
    if(t == 100) {
        levelComplete();
        return;
    }
...
function newTextPopUp(text){
    textPopUp[indexTextPopUp] = new TextPopUp(currentPositionStick['x'] + 5*stickLength, currentPositionStick['y'] - 5,indexTextPopUp);
    textPopUp[indexTextPopUp++].startInterval(text);
}
function TextPopUp(x,y,index){
    ...
    this.startInterval = function(text){
        this.interval = setInterval(function(){textPopUp[index].move(text);},50);
    }
    
    this.move = function(text){
        ctx.fillStyle = "rgba(255,0,0,"+this.fade+")"; 
        ctx.font="15px Arial";
        ctx.clearRect(this.x-5,this.y - 15,200,20);
...
function EndCut(){
    ...
    endCutText.push(new newEndCutText("Game Lead Designer",25,canvas.width/2-130,canvas.height/2,1));
